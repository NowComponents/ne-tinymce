/*! RESOURCE: /scripts/sn-component-custom-tinymce/tinymce.js */
(function () {
    (function (domGlobals) {
    'use strict';
    var noop = function () {
    ...
    experimentalShadowDom: domGlobals.document.head.attachShadow,
    ...
    each(descendants(rootElement, '.mce-offscreen-selection'), remove$1);
    };
    ...
    var selectedCells = descendants(elm, 'td[data-mce-selected],th[data-mce-selected]');
    ...
    sel.removeAllRanges();
    sel.addRange(range);
    each(descendants(Element.fromDom(editor.getBody()), '*[data-mce-selected]'), function (elm) {
    ...
    each(descendants(elm, '*[data-mce-bogus]'), unwrap);
    return get$3(elm);
    };
    ...
    each(descendants(tableElm, 'tr'), function (tr, y) {
    each(descendants(tr, 'td,th'), function (td, x) {
    ...
    var cells = descendants(Element.fromDom(table), 'td,th,caption').map(function (e) {
    return e.dom();
    });
    ...
    if (!this.contextElm) {
    this.contextElm = this.findShadowRoot(this.targetElm);
    }
    ...
    return this.findShadowRoot(node.parentNode);
    },
    getDoc: function () {
    var self = this;
    var win;
    ...
    var className = elm.className ? elm.className.toString() : '';
    return className.indexOf('mce-') !== -1;
    };
    ...
    try {
    return Env.experimentalShadowDom ? getActiveElementHelper(domGlobals.document.activeElement) : domGlobals.document.activeElement;
    } catch (ex) {
    return domGlobals.document.body;
    }
    };
    var getActiveElementHelper = function (element) {
    if (element.shadowRoot) {
    return getActiveElementHelper(element.shadowRoot.activeElement);
    }
    return element;
    };
